import pandas as pd
import numpy as np
# We dont use neural network class from sklearn
# we only use pandas for data manipulation and numpy for math
...
df = pd.read_csv("https://archive.ics.uci.edu/ml/machine-learning-databases/iris/iris.data", names=irisColumns)
...
def train_test_split(input, output, test_size = 0.3, random_state = 0):
    np.random.seed(random_state)
    indices = np.random.permutation(len(input))
    test_size = int(len(input)*test_size)
    test_idx, train_idx = indices[:test_size], indices[test_size:]
    x_train, x_test = input[train_idx,:], input[test_idx,:]
    y_train, y_test = output[train_idx], output[test_idx]
...
# This is an implementation without libraries, but using
# sklearn library the best would be to perform a cross
# validation, to find the average accuracy
